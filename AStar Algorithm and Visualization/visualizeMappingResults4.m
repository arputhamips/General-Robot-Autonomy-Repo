function visualizeMappingResults4(dataStore)
...
h = plot(NaN, NaN, ':', 'Color', [0.5, 0.5, 0], 'LineStyle', ':', 'LineWidth', 2);
legend_handles = [legend_handles, h];
legend_text{end+1} = 'Unknown Optional Walls';
...
% Create a fourth figure for the simplified map visualization as requested
figure('Position', [100, 100, 800, 600], 'Name', 'Simplified Map Visualization');
hold on;

% Draw fixed walls and confirmed existing walls (black)
for i = 1:size(map, 1)
    line([map(i, 1), map(i, 3)], [map(i, 2), map(i, 4)], 'Color', 'k', 'LineWidth', 2);
end

% Draw confirmed existing optional walls (also in black)
for i = 1:size(optWalls, 1)
    if optWallsStatus(i) == 1  % Exists
        line([optWalls(i, 1), optWalls(i, 3)], [optWalls(i, 2), optWalls(i, 4)], ...
            'Color', 'k', 'LineWidth', 2);
    elseif optWallsStatus(i) == 0  % Unknown - red solid line
        line([optWalls(i, 1), optWalls(i, 3)], [optWalls(i, 2), optWalls(i, 4)], ...
            'Color', 'r', 'LineWidth', 2);
    end
    % Non-existing walls (optWallsStatus(i) == -1) are not drawn
end
...
title('Simplified Map Visualization');
xlabel('X (m)');
ylabel('Y (m)');
axis equal;
grid on;

% Add legend for simplified map
legend_handles = [];
legend_text = {};

h = plot(NaN, NaN, 'k-', 'LineWidth', 2);
legend_handles = [legend_handles, h];
legend_text{end+1} = 'Confirmed Walls';

h = plot(NaN, NaN, 'r-', 'LineWidth', 2);
legend_handles = [legend_handles, h];
legend_text{end+1} = 'Unknown Walls';
