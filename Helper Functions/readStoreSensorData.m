...
try 
    ...
    try
        ...
        % depth_array = LidarSensorCreate(Robot);
        dataStore.rsdepth = [dataStore.rsdepth ; toc 0 depth_array];
    catch
        disp('depth error');
    end
    ...
    try
        [Ntag, Z, X, ROT] = RealSenseTag(Robot);
        if ~isempty([Ntag, Z, X, ROT])
            dataStore.beacon = [dataStore.beacon ; repmat(toc,size([Ntag, Z, X, ROT],1),1), [Ntag, Z, X, ROT]];
        end
    catch
        disp('beacon error');
    end
